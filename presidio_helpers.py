...
from presidio_nlp_engine_config import create_nlp_engine_with_transformers, create_nlp_engine_with_azure_ai_language
...
def analyzer_engine(
    ...
    # Add built-in pattern recognizers for MRN/ID/Number labels
    mrn_pattern = Pattern(name="MRN with label", regex=r"MRN:\\s*\\d+", score=0.85)
    mrn_recognizer = PatternRecognizer(supported_entity="MEDICAL_RECORD_NUMBER", patterns=[mrn_pattern], context=["MRN"])
    registry.add_recognizer(mrn_recognizer)

    # Example: add other ID/Number style entities as needed
    id_pattern = Pattern(name="ID with label", regex=r"ID:\\s*\\d+", score=0.85)
    id_recognizer = PatternRecognizer(supported_entity="GENERIC_ID", patterns=[id_pattern], context=["ID"])
...
def redact_pdf(
    ...
    """Redact sensitive entities in a PDF document.

    :param pdf_document: The PDF document to redact.
    :param analyze_results: A list of RecognizerResult objects from the analyzer.
    """
    for page_num in range(len(pdf_document)):
        page = pdf_document[page_num]
        for result in analyze_results:
            # Check if the result is on the current page
            if result.start >= page.rect.x0 and result.end <= page.rect.x1:
                # Find the word in the PDF page
                text_instances = page.search_for(result.word)
