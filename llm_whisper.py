...
def extract_text_from_pdf(file_path, pages_list=None):
    import time
    base_url = os.environ.get("LLMWHISPERER_API_URL", "https://llmwhisperer-api.us-central.unstract.com/api/v2")
    api_key = os.environ.get("LLMWHISPERER_API_KEY", "")
    llmw = LLMWhispererClientV2(base_url=base_url, api_key=api_key)
    try:
        result = llmw.whisper(file_path=file_path, pages_to_extract=pages_list)
        # Handle direct sync response
        if "extracted_text" in result:
            return result["extracted_text"]
        # Async: check status until done, then retrieve
        elif "whisper_hash" in result:
            whisper_hash = result["whisper_hash"]
            max_wait = 60
            wait_time = 0
            while wait_time < max_wait:
                status_info = llmw.whisper_status(whisper_hash=whisper_hash)
                if status_info.get("status") == "processed":
                    retrieve = llmw.whisper_retrieve(whisper_hash=whisper_hash)
                    # May need 'result_text' or just the string
                    return retrieve.get("result_text", str(retrieve))
                elif status_info.get("status") in ["unknown", "delivered"]:
                    break
                time.sleep(3)
                wait_time += 3
            raise Exception(f"Whisper job did not complete in time (waited {wait_time}s)")
        else:
            raise Exception("Unexpected response from LLMWhisperer: {}".format(result))
